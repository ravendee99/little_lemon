...
describe("Booking page", () => {
	...
	test("should have one or more available booking time options", async () => {
		...
		timeOptions.forEach((timeOption) => expect(timeOption.value).toMatch(timeFormat));
	});
	...
	test("should update available booking time options when changing booking date", async () => {
		...
		initialTimeOptions.forEach((timeOption) => expect(timeOption.value).toMatch(timeFormat));
		updatedTimeOptions.forEach((timeOption) => expect(timeOption.value).toMatch(timeFormat));
